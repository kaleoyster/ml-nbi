...
def plot_samples(X_train, Y_train, feature_names, classifier):
    ...
    #plt.show()
    plt.savefig("sample_plot_xg_boost.png", dpi=300)
    ...
    plt.suptitle('3D Partial Dependency Plot (XG Boost)', fontsize=16)

    #plt.show()
    plt.savefig("PDP_plot_xg_boost.png", dpi=300)
...
def xgb_utility(train_x, trainy,
    ...
    _kappa = cohen_kappa_score(prediction, testy,
                              weights='quadratic')
    return _acc, _cm, _cr, _kappa, _auc, fpr, tpr, model, mean_shap_features
...
def main():
    ...
    for state in states:
        ...
        for foldTrainX, foldTestX in kfold.split(X):
            ...
            # Training
            acc, cm, cr, kappa, auc, fpr, tpr, model, mean_shap_features = xgb_utility(trainX, trainy, testX, testy, cols)
